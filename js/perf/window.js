...
function changeWindow(windowNumber) {
  ...
  windowElement.focus(); // ensure the new active window receives focus
  ...
  for (let i = 1; i <= NUMBER_OF_WINDOWS; i++) {
    if (i !== windowNumber) {
      ...
      otherWindow.setAttribute('aria-live', 'polite'); // change aria-live back to polite for inactive windows
    }
  }
