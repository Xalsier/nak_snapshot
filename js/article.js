async function loadHtmlContent(contentAreaId, htmlFile) {
  try {
    ...
    document.getElementById(contentAreaId).src = htmlFile;
  } catch (error) {
...
function handleDropdownChange(contentAreaId) {
  return function () {
    const htmlFile = this.value;
    if (htmlFile) {
      loadHtmlContent(contentAreaId, htmlFile);
    } else {
      document.getElementById(contentAreaId).src = '';
    }
  };
}
