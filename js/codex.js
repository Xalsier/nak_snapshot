...
function fetchAndPopulate(yamlData, selectorId, filterKey, filterValue) {
  fetch('https://nakioku.com/nak_snapshot/yaml/data.yaml').then((response) => response.text()).then((yamlText) => jsyaml.load(yamlText)).then((data) => {
      let path, items = [];
...
if (document.getElementById('collection-list')) {
  fetch('https://nakioku.com/nak_snapshot/yaml/data.yaml')
  ...
  document.getElementById('collection-list').addEventListener('change', function () {const selectedCollection = this.value;
    fetch('https://nakioku.com/nak_snapshot/yaml/data.yaml').then((response) => response.text()).then((yamlText) => jsyaml.load(yamlText)).then((data) => {fetchAndPopulate(data.pages, 'page-list', 'collections', selectedCollection);});});}
