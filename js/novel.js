function populateSelect(yamlData, selectorId) {
  ...
  // Clear the options before adding new ones
  selector.innerHTML = '';
  ...
  fetch('yaml/data.yaml')
    ...
    .then((data) => {
      ...
      if (filterKey === 'volumes') {
        ...
        // loop through each volume to find matching nested items
        for (const [volume, value] of Object.entries(data.chapters.volumes)) {
          if (value.items) {
            items = items.concat(
              value.items.filter((item) => item.key === filterValue)
            );
          }
        }
  ...
  fetch('yaml/data.yaml')
    ...
    .then((data) => {
      ...
      volumeList.forEach((volume) => {
        ...
        document.getElementById('volume-list').appendChild(option);
      });
      // Trigger the initial population of the chapter list
      fetchAndPopulate(data.chapters, 'chapter-list', 'volumes', volumeList[0]);
    });
  document.getElementById('volume-list').addEventListener('change', function () {
    fetchAndPopulate(data.chapters, 'chapter-list', 'volumes', this.value);
  });
}
...
if (document.getElementById('chapter-list')) {
  ...
  // Trigger the initial content display for the first chapter
