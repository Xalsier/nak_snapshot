...
function getWordCount(paragraphs) {
  ...
  return paragraphs.reduce((count, paragraph) => {
    ...
    return count + text.replace(/<[^>]*>/g, '').trim().split(' ').length;}, 0);}
async function fetchNovelData() {
  try {const response = await fetch('../vol_dat.json');
    if (response.ok) {
      ...
      for (const volumeKey in novel) {
        ...
        for (const chapterKey in volume.chapters) {
          ...
          } else {throw new Error('Failed to fetch chapter: ' + chapterKey);}}
    ...
    }} catch (error) {console.error('Error fetching novel data:', error);}}
fetchNovelData();
...
class NovelState {
    ...
    renderContent() {
      ...
      if (!this._contentWarningAccepted) {
        ...
        this.chapterContainer.appendChild(contentWarning);return;}
      if (this._volumeCardVisible) {
        const volumeCard = createVolumeCard(novel[this.currentVolume], this);
        this.chapterContainer.appendChild(volumeCard);return;}
